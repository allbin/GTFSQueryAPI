...
func (r *Repository) GetStops(lat string, lon string, radius string, maxstops string) (*sql.Rows, error) {
	return r.Db.Query(
		fmt.Sprintf("SELECT st_distance(st_point(stop_lat, stop_lon), st_point(%s, %s)) as distance, s.stop_id as id, arrival_time, departure_time, stop_name as name, stop_lat as lat, stop_lon as lon,"+
			...
			" order by distance limit %s)  AND ((date(current_timestamp + interval '- 4 hours') = cd.date"+
			" OR date(current_timestamp + interval '20 hours') = cd.date)) order by date_string;", lat, lon, lat, lon, radius, maxstops))
}
